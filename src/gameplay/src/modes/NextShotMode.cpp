...
void NextShotMode::OnStart() {
    ...
    const auto activeSpawnOffset =
        _RandomSpawnOffset(minimumDistance, std::nullopt);
    ...
    const auto previewSpawnOffset =
        _RandomSpawnOffset(minimumDistance, activeSpawnOffset);
    ...
    m_shotHitSubscriptionId =
        GetEventBus().Subscribe([this](const core::events::CoreEvent& event) {
            ...
            if (!m_previewSpawnOffset.has_value()) {
                const double minimumDistance = _MinimumSpawnOffsetDistance();
                const auto fallbackActive =
                    _RandomSpawnOffset(minimumDistance, m_activeSpawnOffset);
                if (!_SpawnActiveTargetAtOffset(fallbackActive)) {
                    return;
                }

                const auto fallbackPreview =
                    _RandomSpawnOffset(minimumDistance, fallbackActive);
            ...
            const double minimumDistance = _MinimumSpawnOffsetDistance();
            const auto nextPreviewOffset =
                _RandomSpawnOffset(minimumDistance, nextActiveOffset);
            _SpawnPreviewTargetAtOffset(nextPreviewOffset);
        });
...
bool NextShotMode::_SpawnActiveTargetAtOffset(
    const std::array<double, 2>& spawnOffset) {
...
bool NextShotMode::_SpawnPreviewTargetAtOffset(
    const std::array<double, 2>& spawnOffset) {
    if (m_previewTargetObjectId != 0) {
        GetSceneCommandSink().DestroyTarget(m_previewTargetObjectId);
        m_previewTargetObjectId = 0;
    ...
    m_previewTargetObjectId =
        GetSceneCommandSink().SpawnSphereTarget(previewRequest);
    if (m_previewTargetObjectId == 0) {
        m_previewSpawnOffset.reset();
        GetLogger().Warning("gameplay",
                            "NextShotMode failed to spawn preview target");
...
std::array<double, 2> NextShotMode::_RandomSpawnOffset(
    ...
    for (std::size_t attempt = 0; attempt < MAX_RANDOM_SAMPLE_ATTEMPTS;
        ...
        const double candidateDistance =
            _PlanarDistance(candidateOffset, *avoidOffset);
...
double NextShotMode::_MinimumSpawnOffsetDistance() const {
    ...
    return std::clamp(cappedMinimumSpawnOffsetDistance, 0.0,
                      maxSpawnOffsetDistance);
}
