...
QString UiViewModel::_LocalizedModeName(const ModeDescriptor& mode) const {
    if (m_settings.ui.language == persistence::UiLanguage::Turkish) {
        if (mode.id == "static_sphere") {
            return QStringLiteral("Sabit Küre");
        }

        if (mode.id == "gridshot") {
            return QStringLiteral("GridShot");
        }

        if (mode.id == "next_shot") {
            return QStringLiteral("Next Shot");
        }

        if (mode.id == "strafing_targets") {
            return QStringLiteral("Strafing Hedefler");
        }

        if (mode.id == "tracking_targets") {
            return QStringLiteral("Tracking Hedefler");
        }
    }

    return QString::fromStdString(mode.displayName);
}

QString
UiViewModel::_LocalizedModeDescription(const ModeDescriptor& mode) const {
    if (m_settings.ui.language == persistence::UiLanguage::Turkish) {
        if (mode.id == "static_sphere") {
    ...
    return QString::fromStdString(mode.description);
}
...
QString UiViewModel::_LocalizedModeSettingName(
    ...
    if (m_settings.ui.language == persistence::UiLanguage::Turkish) {
        if (modeId == "gridshot") {
            if (setting.key == "grid_rows") {
                return QStringLiteral("Grid Satır");
            }

            if (setting.key == "grid_columns") {
                return QStringLiteral("Grid Sütun");
            }

            if (setting.key == "active_target_count") {
                return QStringLiteral("Aktif Hedef");
            }
        }

        if (modeId == "strafing_targets") {
            if (setting.key == "speed") {
                return QStringLiteral("Hız");
            }
        }
    ...
    return QString::fromStdString(setting.displayName);
}
...
void UiViewModel::_RebuildI18n() {
    if (m_settings.ui.language == persistence::UiLanguage::Turkish) {
        m_i18n = {
            ...
            {"menuSubtitle",
             QStringLiteral("Mod seç, ayarları düzenle ve antrenmana başla.")},
            ...
            {"modeSubtitle",
             QStringLiteral("Antrenmana başlamadan önce oyun modunu belirle.")},
            ...
            {"resultNotSavedHint",
             QStringLiteral(
                 "Exit Training ile çıkıldığı için sonuç geçmişe eklenmedi.")},
            {"resultInvalidHint",
             QStringLiteral("Bu sonuç geçersiz olduğu için geçmişe eklenmedi.")},
            ...
            {"resultInvalidReasonAccuracy",
             QStringLiteral("Doğruluk 0% veya 100%'ün üstünde")},
            {"resultInvalidReasonUnknown", QStringLiteral("Geçersiz metrik")},
            {"resultLiveHint",
             QStringLiteral("Antrenman bitmeden önce seçili mod ve canlı metrikler "
                            "burada gösterilir.")},
            ...
            {"trainingSubtitle",
             QStringLiteral("Canlı metrikleri takip et ve performansını yükselt.")},
            ...
            {"trainingCaptureHint",
             QStringLiteral("İpucu: Nişan kontrolü için eğitim sırasında fare "
                            "yakalama açıktır.")},
            {"trainingWaitForClick",
             QStringLiteral("Antrenmanı başlatmak için sahneye sol tık yap.")},
            {"trainingCountdown", QStringLiteral("Başlıyor")},
            {"trainingPausedTitle", QStringLiteral("Antrenman Duraklatıldı")},
            {"trainingPausedSubtitle",
             QStringLiteral("Sayaçlar ve mod güncellemeleri beklemede.")},
            ...
            {"statsSubtitle",
             QStringLiteral(
                 "Geçmiş oturumları incele ve en iyi sonucunla kıyasla.")},
            ...
            {"settingsSubtitle",
             QStringLiteral("Tüm değişiklikler anında uygulanır ve kaydedilir.")},
            ...
            {"settingsCrosshairCenterDotSize",
             QStringLiteral("Merkez Nokta Boyutu")},
    ...
    m_i18n = {
        ...
        {"menuSubtitle",
         QStringLiteral("Choose a mode, adjust settings, and start training.")},
        ...
        {"modeSubtitle",
         QStringLiteral("Pick your training mode before starting a run.")},
        ...
        {"resultNotSavedHint",
         QStringLiteral("Session exited via Exit Training, so this result was "
                        "not added to history.")},
        {"resultInvalidHint",
         QStringLiteral("This result is invalid and was not added to history.")},
        {"resultInvalidReasonReaction",
         QStringLiteral("Reaction time is 0.0 ms")},
        {"resultInvalidReasonHits", QStringLiteral("Hit count is 0")},
        {"resultInvalidReasonScore", QStringLiteral("Score is negative")},
        {"resultInvalidReasonAccuracy",
         QStringLiteral("Accuracy is 0% or above 100%")},
        {"resultInvalidReasonUnknown", QStringLiteral("Invalid metric state")},
        {"resultLiveHint", QStringLiteral("Selected mode and live metrics are "
                                          "shown here until the session ends.")},
        ...
        {"trainingSubtitle",
         QStringLiteral("Track real-time metrics and improve consistency.")},
        ...
        {"trainingCaptureHint",
         QStringLiteral("Hint: mouse capture is enabled while training for "
                        "FPS-style aim control.")},
        {"trainingWaitForClick",
         QStringLiteral("Left click inside the scene to start training.")},
        {"trainingCountdown", QStringLiteral("Starting")},
        {"trainingPausedTitle", QStringLiteral("Training Paused")},
        {"trainingPausedSubtitle",
         QStringLiteral("Mode updates and counters are paused.")},
        ...
        {"statsSubtitle",
         QStringLiteral(
             "Review session history and compare against your best run.")},
        {"statsBestSession", QStringLiteral("Best Session For This Mode")},
        {"statsNoBest",
         QStringLiteral("No sessions recorded for this mode yet.")},
        ...
        {"settingsSubtitle",
         QStringLiteral("All changes are applied and saved instantly.")},
        ...
        {"settingsCrosshairHorizontalLength",
         QStringLiteral("Horizontal Length")},
